...
def _get_feature_names_from_config(config: Dict[str, Any], df: Optional[pl.DataFrame] = None) -> List[str]:
    ...
    if "feature_columns" in config and config["feature_columns"]:
        ...
        raise ValueError("Cannot determine feature names from config. Provide feature_columns in config or pass df parameter.")


def _get_categorical_indices_from_config(config: Dict[str, Any], feature_names: List[str]) -> List[int]:
...
def get_feature_importance(
    ...
    feature_names: Optional[List[str]] = None,
    importance_type: Literal['PredictionValuesChange', 'LossFunctionChange', 'FeatureImportance'] = 'PredictionValuesChange',
...
def plot_feature_importance(
    ...
    if backend == "plotly":
        ...
        # Update layout
        plot_title = title if title else f'Top {top_n} Features by {importance_result.importance_type}'
        fig.update_layout(
            title=plot_title,
...
def calculate_shap_values(
    ...
    if len(feature_names) != shap_values.shape[1]:
        raise ValueError(f"Length of feature_names ({len(feature_names)}) does not match number of features ({shap_values.shape[1]})")
...
def plot_shap_waterfall(
    ...
    explanation = shap.Explanation(
        ...
        base_values=shap_result.base_value[sample_index] if isinstance(shap_result.base_value, np.ndarray) else shap_result.base_value,
...
def plot_shap_force(
    ...
    base_value = shap_result.base_value[sample_index] if isinstance(shap_result.base_value, np.ndarray) else shap_result.base_value
    ...
    if interaction_feature is not None and interaction_feature != 'auto':
        if isinstance(interaction_feature, str):
            if interaction_feature not in shap_result.feature_names:
                raise ValueError(f"Interaction feature '{interaction_feature}' not found in feature_names")
...
class PartialDependenceResult:
    ...
    def to_polars(self) -> pl.DataFrame:
        ...
        return pl.DataFrame({
            ...
            'partial_dependence': self.pd_values
        })
...
def plot_multiple_partial_dependence(
    ...
    if backend == "plotly":
        ...
        fig = make_subplots(
            ...
            subplot_titles=[feature_names[idx] if feature_names else f'feature_{idx}' 
...
def create_evaluation_report(
    ...
    if output_dir:
        ...
        if backend == "matplotlib":
            fig_importance.savefig(f'{output_dir}/feature_importance.png', dpi=300, bbox_inches='tight')
        else:
    ...
    fig_pd = plot_multiple_partial_dependence(
        model, X, top_feature_indices, feature_names,
    ...
    print(f"Evaluation report complete. Generated {len([k for k in results.keys() if k.startswith('fig_')])} figures.")
