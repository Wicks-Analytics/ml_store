...
def load_config(config_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    if config["model_type"] not in ["classifier", "regressor"]:
        raise ValueError(f"Invalid model_type: {config['model_type']}. Must be 'classifier' or 'regressor'")
    ...
    try:
        if data_type == "csv":
            ...
            if "db_type" not in config:
                raise ValueError("Missing required field for SQL: db_type (must be 'snowflake' or 'mssql')")
            ...
            if db_type == "snowflake":
                ...
                raise ValueError(f"Unsupported db_type: {db_type}. Supported types: snowflake, mssql")
        else:
            raise ValueError(
        ...
        if data_type == "sql":
            ...
            raise RuntimeError(f"Error reading {data_type} file from {data_path_or_query}: {str(e)}")
...
def prepare_features(
    ...
    if "exposure_column" in config and config["exposure_column"]:
        if config["exposure_column"] not in df.columns:
            raise ValueError(f"Exposure column '{config['exposure_column']}' not found in DataFrame")
...
def assign_split(
    ...
    if "split_column" in config and config["split_column"]:
        ...
        if train_df.height == 0:
            raise ValueError(f"No rows found for train split with value '{split_values.get('train')}'")
        if test_df.height == 0:
            raise ValueError(f"No rows found for test split with value '{split_values.get('test')}'")
        ...
        if "holdout" in split_values:
            ...
            if holdout_df.height == 0:
                raise ValueError(f"No rows found for holdout split with value '{split_values.get('holdout')}'")
        ...
        use_holdout = has_holdout_in_config or has_holdout_in_params or (
            "train_ratio" not in config and "test_ratio" not in config and 
            train_ratio is None and test_ratio is None
        )
        
        if use_holdout:
            # 3-way split
            final_train_ratio = config.get("train_ratio", train_ratio if train_ratio is not None else 0.7)
            final_test_ratio = config.get("test_ratio", test_ratio if test_ratio is not None else 0.15)
            final_holdout_ratio = config.get("holdout_ratio", holdout_ratio if holdout_ratio is not None else 0.15)
            final_random_seed = config.get("random_seed", random_seed if random_seed is not None else 42)
            ...
            # 2-way split
            final_train_ratio = config.get("train_ratio", train_ratio if train_ratio is not None else 0.8)
            final_test_ratio = config.get("test_ratio", test_ratio if test_ratio is not None else 0.2)
            final_random_seed = config.get("random_seed", random_seed if random_seed is not None else 42)
...
def create_modelling_data(
    ...
    Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str], List[int], Optional[np.ndarray], Optional[np.ndarray]],
    Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str], List[int], Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]
]:
    """
    ...
    if len(split_result) == 2:
        ...
        X_train, y_train, feature_names, cat_indices, exposure_train = prepare_features(train_df, config, is_training=True)
        X_test, y_test, _, _, exposure_test = prepare_features(test_df, config, is_training=True)
        
        return X_train, y_train, X_test, y_test, feature_names, cat_indices, exposure_train, exposure_test
    else:
        # 3-way split
        ...
        X_train, y_train, feature_names, cat_indices, exposure_train = prepare_features(train_df, config, is_training=True)
        X_test, y_test, _, _, exposure_test = prepare_features(test_df, config, is_training=True)
        X_holdout, y_holdout, _, _, exposure_holdout = prepare_features(holdout_df, config, is_training=True)
        
        return X_train, y_train, X_test, y_test, X_holdout, y_holdout, feature_names, cat_indices, exposure_train, exposure_test, exposure_holdout
